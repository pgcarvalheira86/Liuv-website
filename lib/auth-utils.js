...
export function getCookieDomain(host) {
  ...
  if (h === 'localhost' || h === '127.0.0.1' || h.endsWith('.local')) return undefined;
  if (h === 'liuv.io' || h.endsWith('.liuv.io')) return '.liuv.io';
  return undefined;
}

export function setAuthCookie(token, opts = {}) {
  const base = {
    httpOnly: true,
    secure: true,
    sameSite: 'Lax',
  ...
  if (opts.domain != null) base.domain = opts.domain;
  return serialize(COOKIE_NAME, token, base);
}
...
export function clearAuthCookie(opts = {}) {
  ...
  if (opts.domain != null) base.domain = opts.domain;
  return serialize(COOKIE_NAME, '', base);
}
...
let blobsStoreOptions = null;
export function setBlobsStoreOptions(opts) {
  blobsStoreOptions = opts && (opts.siteID || opts.token) ? opts : null;
}

export function setBlobsContextFromEvent(event) {
  if (!event?.blobs || !event?.headers) return;
  try {
    const data = JSON.parse(Buffer.from(event.blobs, 'base64').toString());
    const siteID = process.env.NETLIFY_SITE_ID || event.headers['x-nf-site-id'] || event.headers['X-Nf-Site-Id'];
    if (siteID && data?.token) {
      setBlobsStoreOptions({ siteID, token: data.token });
    }
  } catch (_) {}
}
...
async function storeWithBlobFallback(blobName, fileName) {
  ...
  let blob = null;
  try {
    const storeOpts = { name: blobName, consistency: 'strong' };
    if (blobsStoreOptions?.siteID && blobsStoreOptions?.token) {
      storeOpts.siteID = blobsStoreOptions.siteID;
      storeOpts.token = blobsStoreOptions.token;
    }
    blob = getStore(storeOpts);
    console.log('[AUTH] Using Blobs store:', blobName);
  } catch (e) {
    console.log('[AUTH] Blobs unavailable:', blobName, e.message || e);
    if (isProduction) {
      throw new Error(
        `Blobs store "${blobName}" is not available in production. Ensure Netlify Blobs is enabled and connectLambda(event) is called at the start of your function handler.`
      );
    }
    return fallback;
  }
  return {
    get: async (key, opts) => {
      try {
        const val = await blob.get(key, opts);
        return val ?? undefined;
      } catch {
        if (isProduction) throw new Error(`Blobs read failed for store "${blobName}"`);
    ...
    setJSON: async (key, val) => {
      try {
        ...
        console.log(`[AUTH] Saved to Blobs store: ${blobName}`);
      } catch (e) {
        if (isProduction) throw new Error(`Blobs write failed for store "${blobName}": ${e.message || e}`);
...
async function getOnboardingStore() {
  return storeWithBlobFallback('onboarding', 'onboarding.json');
}
...
export async function setOnboarding(userId, data) {
  if (!userId) return null;
  const store = await getOnboardingStore();
  const key = `user:${userId}`;
  const payload = {
...
export async function linkStripeCustomer(email, stripeCustomerId, plan, name = '') {
  ...
  await planStore.setJSON(`stripe:${stripeCustomerId}`, { email: email.toLowerCase().trim(), plan });
...
export async function getStripeCustomerMapping(stripeCustomerId) {
  if (!stripeCustomerId) return null;
  const planStore = await getPlanStore();
  try {
    const data = await planStore.get(`stripe:${stripeCustomerId}`, { type: 'json' });
    return data ?? null;
  } catch {
    return null;
  }
...
export async function syncUserPlanFromStripeCustomer(stripeCustomerId, plan) {
  const mapping = await getStripeCustomerMapping(stripeCustomerId);
  if (mapping?.email && plan) {
    return updateUser(mapping.email, { plan });
  }
  return null;
}
