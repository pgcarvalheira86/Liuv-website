...
const SITE_URL = (() => {
  if (process.env.AUTH_SITE_URL) return process.env.AUTH_SITE_URL.replace(/\/$/, '');
  const u = process.env.URL || process.env.DEPLOY_PRIME_URL || 'https://liuv.io';
  const base = u.replace(/\/$/, '');
  if (base.includes('liuv.io')) return 'https://liuv.io';
  return base;
})();
